...
def run(args):
            ...
            # load sparse solutions
                ...
                res = pickle.load(
                    open(
                        ...
                        'rb'))
                bidual, opt, lmbdas = np.array(res['bd']), np.array(
                    res['opt']), res['lmbdas']
                if vary == 'lmbda':
                    lmbdas = range(1, len(bidual) + 1)                   
                zeta_r, zeta = np.abs(np.array(res['zeta_r'])), np.abs(np.array(res['zeta']))
                xvals, bnd, bnd_label = sf_bound(l0reg,
                                                 'constr',
                                                 lmbdas,
                                                 bidual,
                                                 hyperparams[counter],
                ...
                if l0reg == 'pen':
                    ...
                    # OPT value
                    mean = np.array(
                        [np.mean(o + z) for o, z in zip(opt, zeta)])
    ...
    plt.savefig(f'{prefix_str}/figures/nr_{n}_{m}_{r}_{loss}_{sparsity}_{gamma}_{vary}_{args.param_grid}.pdf')
    plt.close()
