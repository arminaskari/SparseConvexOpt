...
X = scale(PolynomialFeatures(3, include_bias=False).fit_transform(
    diabetes.data))
print(X.shape)
y = diabetes.target
...
for i in range(20):
    ...
    print(np.linalg.norm(Xtilde.T @ ytilde - X_train.T @ y_train))
    assert np.linalg.norm(Xtilde.T @ ytilde - X_train.T @ y_train) <= 1e-9
        
    # sparse model solutions
    max_k = 50
    supp_du, cost_du = sm.cvx_dual(D, L, -X_train.T @ y_train, 
                                  list(range(1,max_k)),
                                  verbose=True, solver='scs')
...
filename = '../results/diabetes_' + str(rank) + '.pkl'
