...
def vary_lambda(params, ranks, loss, l0reg, gamma):
    ...
    n, m, r_eff = params["Xtrue"].shape[0], params["Xtrue"].shape[1], params["rank"]
    if l0reg == "constr":
    ...
    for r in ranks:
        ...
        for l in lmbdas:
            # fit model
            clf = SparseModel(
                loss=loss, l0reg=l0reg, l2reg="constr", lmbda=l, gamma=gamma
            )
        ...
        pickle.dump(
            res,
            open(
                f"/Users/aaskari/github-repos/SparseConvexOpt/results/rank_bounds/l0{l0reg}/mpqa-res-{loss}_{gamma}_rank{r}.pkl",
                "wb",
            ),
        )
...
def vary_rank(params, lmbdas, loss, l0reg, gamma):
    ...
    for l in lmbdas:
        ...
        for r in range(1, np.min(params["Xtrue"].shape), 5):
            ...
            print(d_ast, clf.bdcost_)
            print(np.abs(d_ast - clf.bdcost_) / np.abs(clf.bdcost_))
            assert np.abs(d_ast - clf.bdcost_) / np.abs(clf.bdcost_) <= 8e-4
        ...
        pickle.dump(
            res,
            open(
                f"/Users/aaskari/github-repos/SparseConvexOpt/results/rank_bounds/l0{l0reg}/leu-res-{loss}_{gamma}_lmbda{l}.pkl",
                "wb",
            ),
        )
