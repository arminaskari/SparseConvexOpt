...
def cvx_qp_constr(Q, p, beta, verbose=False):
    ...
    prob.solve(
        solver=cp.ECOS, verbose=verbose, abstol=abstol, reltol=reltol, feastol=feastol
    )
...
def l0_penalized_bidual_cvx(D, L, p, g, verbose=False, solver="ecos"):
    ...
    for gg in g:
        ...
        cost = (
            0.5 * cp.sum_squares(L.T @ cp.sum(x, 1))
            + cp.sum(cp.sum(t, 1))
            - 4 * gg * m
            + 0 * c
        )
...
def socp_bidual(D, L, p, k, beta, verbose=False, solver="ecos"):
    ...
    for b, kk in zip(beta, k):
        ...
        # constraints
        constr = [L.T @ x == z, cp.norm(x, 1) <= kk * b, cp.norm(x, "inf") <= b]
...
def socp_dual(D, L, p, k, beta, verbose=False, solver="ecos"):
    ...
    for b, kk in zip(beta, k):
        ...
        cost = (
            -b * cp.sum_largest(alpha, kk) - (mu[-1] + mu[-2] + chi[-1] + chi[-2]) / 2
        )
...
def l0_penalized_bidual_bfgs(D, L, p, g, veerbose=False):
    ...
    for gg in g:
        ...
        x0 = np.ones(m)
        options = {"disp": False}
        res = minimize(primal, x0, jac=True, method="L-BFGS-B", options=options)
