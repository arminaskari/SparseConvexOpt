...
if __name__ == "__main__":
    ...
    for l2 in l2reg:
        for l0 in l0reg:
            files = glob.glob(f"{prefix_str}/results/synthetic/l2{l2}/l0{l0}/*.pkl")
            if len(files) > 0:
                for f in files:
                    ...
                    saved_plots = glob.glob(f"{prefix_str}/figures/{save_str}.pdf")
                    # plot results
                    plt.figure()
                    if l0 == "constr":
                    ...
                    if l0 == "pen":
                        plt.semilogx(
                            1 / lmbdas, bidual, "b-", alpha=0.7, label=bd_label
                        )
                        plt.semilogx(1 / lmbdas, mean, "r-", alpha=0.7, label=r"OPT")
                        plt.fill_between(
                            1 / lmbdas, mean + std, mean - std, color="r", alpha=0.3
                        )
                        plt.semilogx(1 / xvals, bnd, "k--", label=bnd_label)
                    else:
                        plt.plot(lmbdas, bidual, "b-", alpha=0.7, label=bd_label)
                    ...
                    if l2 == "constr" and low_rank == "soft":
                        ...
                        if l0 == "constr":
                            ...
                            plt.xlabel(r"$\lambda$")
                        plt.legend()
                        plt.savefig(f"{prefix_str}/figures/nr_{save_str}.pdf")
                        plt.close()
